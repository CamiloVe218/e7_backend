...
function Overline({ children, count }: { children: React.ReactNode; count?: number }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <p className="text-[10px] font-bold tracking-[0.1em] uppercase text-gray-400">{children}</p>
      {count !== undefined && (
...
export default function ProviderDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      ...
      {myActive && (
        <div>
          <Overline>Servicio activo</Overline>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 rounded-xl border border-dashed border-gray-200 bg-white">
            ...
            <p className="text-sm text-gray-400">No hay solicitudes disponibles</p>
            <p className="text-xs text-gray-300 mt-1">Las nuevas aparecerán aquí en tiempo real</p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {available.map(req => (
